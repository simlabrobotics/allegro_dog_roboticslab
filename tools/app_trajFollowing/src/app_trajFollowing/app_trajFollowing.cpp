...
#else
string_type control_pid_path = _T("controls/control_AllegroDogNRLTrajFollowingRT.xdl");
#endif
...
void Demo_TrajFollowing_Position();
...
void ServoOn()
{
	...
	{
		if (devMotor[i])
			devMotor[i]->command(ALLEGRODOGMOTOR_CMD_OPMODE_POS);
	}
	...
	if (devCAN[0])
		devCAN[0]->command(CAN_CMD_OPMODE_POSITION, 0, NULL, JDOF);
	if (devCAN[1])
		devCAN[1]->command(CAN_CMD_OPMODE_POSITION, 0, NULL, JDOF);
}
...
{
	if (control_pid)
		control_pid->command(CMD_TEST_MODE_POSITION_SIN);
}
