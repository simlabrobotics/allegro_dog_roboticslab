...
#else
string_type control_pid_path = _T("controls/control_AllegroDogNRLTunePIDRT.xdl");
#endif
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if (sys)
	{
		...
		rdaqAddData(pid_gyro, control_pid, 200);*/
	}
...
void MyKeyboardHandler(int key, void* data)
{
	...
	case VK_DOWN:
		if (control_pid)
			control_pid->command(RCMD_CHANGE_TASK, key);
		break;
	...
	case VK_9:
		if (control_pid)
			control_pid->command(RCMD_SELECT_AXIS, key-VK_1);
		break;
	case VK_0:
		if (control_pid)
			control_pid->command(RCMD_SELECT_AXIS, 9);
		break;
	case VK_SUBTRACT:
		if (control_pid)
			control_pid->command(RCMD_SELECT_AXIS, 10);
		break;
	case VK_ADD:
		if (control_pid)
			control_pid->command(RCMD_SELECT_AXIS, 11);
		break;

	case VK_W:
		if (control_pid)
			control_pid->command(RCMD_INC_Kp, 1);
		break;
	case VK_S:
		if (control_pid)
			control_pid->command(RCMD_INC_Kp, -1);
	...
	case VK_E:
		if (control_pid)
			control_pid->command(RCMD_INC_Ki, 1);
		break;
	case VK_D:
		if (control_pid)
			control_pid->command(RCMD_INC_Ki, -1);
	...
	case VK_R:
		if (control_pid)
			control_pid->command(RCMD_INC_Kv, 1);
		break;
	case VK_F:
		if (control_pid)
			control_pid->command(RCMD_INC_Kv, -1);
	...
	case VK_T:
		if (control_pid)
			control_pid->command(RCMD_INC_InvL2, 1);
		break;
	case VK_G:
		if (control_pid)
			control_pid->command(RCMD_INC_InvL2, -1);
...
void PrintInstruction()
{
	...
	printf("\n");
	printf("Up: increase amplitude of sine trajectory\n");
	printf("Down: decrease amplitude of sine trajectory\n");
	printf("Left: decrease frequency of sine trajectory\n");
	printf("Right: increase frequency of sine trajectory\n");
	printf("\n");
	printf("1~9, 0, -, =: Select a joint(1st ~ 12th)\n");
	printf("\n");
	printf("W: increase Kp\n");
	printf("S: decrease Kp\n");
	printf("E: increase Ki\n");
	printf("D: decrease Ki\n");
	printf("R: increase Kd\n");
	printf("F: decrease Kd\n");
	printf("T: increase InvL2\n");
	printf("G: decrease InvL2\n");
	printf("**********************************************\n");
}
